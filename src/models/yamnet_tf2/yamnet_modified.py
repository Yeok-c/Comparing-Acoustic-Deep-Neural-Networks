...
class Yamnet_modified():
  ...
  def model(self):
    ...
    net = features
    # Feed data in as (batch, row, col, 1) because most data augmentation algos are written for this
    # net = layers.Reshape(
    #     (self.params.patch_frames, self.params.patch_bands, 1),
    #     input_shape=(self.params.patch_frames, self.params.patch_bands))(features)
    ...
    embeddings = layers.Dropout(0.4)(embeddings)
    embeddings = layers.Dense(1024)(embeddings)
    embeddings = layers.Dropout(0.2)(embeddings)
    embeddings = layers.Dense(64)(embeddings)
    
    # predictions = layers.Dense(self.num_classes, activation='softmax')(embeddings
    logits = layers.Dense(units=6, use_bias=True)(embeddings)
    predictions = layers.Activation(activation=self.params.classifier_activation)(logits)
